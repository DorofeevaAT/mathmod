...
Эволюция численностей армий x и y происходит вдоль гиперболы, заданной этим уравнением. По какой именно гиперболе пойдет война, зависит от начальной точки.
...
В начальный момент времени страна $X$ имеет армию численностью 40 000 человек, а в распоряжении страны $Y$ армия численностью в 69 000 человек.
...
$$
 \begin{cases}
	\frac{dx}{dt}= -0,37x(t) - 0,73y(t) + 2sin(6t)
	\\   
	\frac{dy}{dt}= -0,28x(t)y(t) - 0,82y(t) + 2cos(4t)
 \end{cases}
$$
...
```
model lab3
  parameter Real a(start=0.331);
  parameter Real b(start=0.771);  
  parameter Real c(start=0.401);
  parameter Real h(start=0.731);
  Real x(start=40000);
  Real y(start=69000);
equation
...
```
model lab03
parameter Real a(start=0.37);
parameter Real b(start=0.73);
parameter Real c(start=0.28);
parameter Real h(start=0.82);
Real x(start=40000);
Real y(start=69000);
equation
